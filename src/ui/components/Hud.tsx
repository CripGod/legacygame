        ...
        <div className="hud-mid">
          {stand && (
            ...
            <span className={`sob-seat ${ramp ? 'ramp' : ''}`} style={{ '--heat': heat } as React.CSSProperties}>
              <button className={`sob ${stand.on ? 'on' : ''} ${stand.stood ? 'stood' : ''} ${stand.between ? 'between' : ''} ${stand.flash ? 'ftue-flash' : ''} ${stand.slam ? 'slam' : ''} ${stand.urge ? 'urge' : ''} ${stand.opening && !stand.disabled && !stand.on && !stand.stood ? 'opening' : ''}`} disabled={stand.disabled} onClick={stand.onToggle} aria-label={stand.on ? 'Standing on Business' : 'Stand on Business'} title={view.pendingRaises.length ? HINTS.stakesPending : HINTS.stakes}>
