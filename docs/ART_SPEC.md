...
| Card frame | `public/art/frames/<kind>.webp` and `<kind>-sm.webp` | 1103 × 1426 and 552 × 714, WebP with alpha | The designer's frame with the art window knocked out: `character` (blue) and `event` (purple; the 1086 × 1448 export is fitted into the 1103 × 1426 box, centred, so both frames share one layout). See `docs/card-template.md`. |
