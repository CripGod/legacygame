...
- **Nine turns** (ten after Stand on Business) with 22-card decks: four in the opening hand and one draw every turn. (Seven turns and 18 cards until build 126: self-play showed the 18-card deck running dry in most nine-turn matches, so the deck grew with the clock.) Hands hold at most 7 cards; a draw into a full hand is discarded (logged in the turn log).
...
- **Anansi's Web** is a Location that is never drawn: Anansi's Reveal draws a card and retells the Location he is played at as his Web, where the small win against the large (Characters costing 1 or less gain +2 Influence, Characters costing 3 or more lose 1, for both players). Characters, Influence and Threats stay; a Mob that was coming to a retold Greenwood still comes.
...
A 120-match self-play run at nine turns with the Energy cap gives roughly 48% / 52% for Railroad vs Black Star (Mirror vs Mirror 53% / 47%, so the seat is fair), 5.5 Location lead changes per match and 0.35 final-turn flips. The deck additions were chosen against this run: Black Star's first four (Bud Billiken, Herndon, Dave the Potter, Colvin) lost 67% of matches to Railroad's, so OG, Madam C.J. Walker and Oshun took three of the seats.
