...
- **Every card is somebody, or something, that existed, with two deliberate exceptions.** The invented people are gone: the Newsboy is John Russwurm (Freedom's Journal, 1827), the Barber is Alonzo Herndon, the Block Captain is Callie House, the Neighbor Kid is Bud Billiken (the Chicago Defender's 1923 mascot), Karen is Roger Taney, the Pullman Porter is the Sleeping Car Porters (the 1925 Brotherhood) and the Cookout is the Stroll (State Street, Chicago, 1910s–20s). Brand characters and institutions count as long as they are real history. The Organizer and the OG stay by request as archetypes (no single person is meant). Chairteenth stays too: it is the Montgomery riverfront brawl of 5 August 2023, real but recent and with living participants, so nobody is named on the card. The public-domain line is otherwise people dead before about 1930; Henry Ossawa Tanner (d. 1937) and George Washington Carver (d. 1943) are the flagged exceptions. Threats and Events are conditions, not people, and keep their names.
...
- **Gatherings** are Characters that are never in a deck. **The Stroll** (2/1) arrives at Great Migration, Inside if there is room, once you have three Established Characters there, and only in about half of matches (rolled when the match is created); its other Established friends get +1 Influence and your arrivals there are Ready at once. **Chairteenth** (1/3, +1 Force to friendly confronters) arrives Ready at both players' Gates the moment Juneteenth is revealed. Each arrival opens a fanfare card. If a card ever returns a Gathering to hand, it can be replayed like any Character.
