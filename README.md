...
- **Gatherings** are Characters that are never in a deck. **The Cookout** (2/1) arrives at Great Migration, Inside if there is room, once you have three Established Characters there, and only in about half of matches (rolled when the match is created); its other Established friends get +1 Influence and your arrivals there are Ready at once. **Chairteenth** (1/3, +1 Force to friendly confronters) arrives Ready at both players' Gates the moment Juneteenth is revealed. Each arrival opens a fanfare card. If a card ever returns a Gathering to hand, it can be replayed like any Character.
