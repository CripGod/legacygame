...
const missing = {
  ...
  'locations (night)': LOCATIONS.filter((l) => !has('locations', `${l.id}_night`)).map((l) => ({ ...l, id: `${l.id}_night`, blurb: `Night version of ${l.name}: same framing, after dark. Used on even turns.${l.curfew ? ' This Location has a curfew: show it shut, lit windows, nobody out.' : ''}` })),
};
const spec: string[] = [];
...
spec.push('| Location (night) | Same as Location, file name `<id>_night` | 1024×1024 JPG | Replaces the Location background on even turns (night). | Same composition as the day image so the swap reads as time passing, not a new place: moon or lamplight, windows lit, streets empty. Curfew Locations should look shut. |');
